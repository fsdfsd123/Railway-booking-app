...
class MyticketViewcontroller: BookingViewController {
...
//    }
    ...
    override func viewDidLoad() {
        ...
        self.headerRouteInformationView.isHidden = true
...
//                        [weak self] in
            ...
            for i in Myticket{
                self.trainSchedules.append(i.Train!)
            }
            print("-----------")
            print(trainSchedules)
            mainTableView.reloadData()
...
extension MyticketViewcontroller: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.setupValue(time: "\(fromTimeString) – \(toTimeString)",
            ...
            price: "from \((cheapestPrice ?? 50)/50 )",
...
//        let prefix = sessionId.substring(to: index)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let trainSchedule = trainSchedules[indexPath.row]
        let seat = Myticket[indexPath.row].seat
        
        
        let bookingConfirmationVC = RecordDetailViewController()
        //bookingConfirmationVC.seatClassType = seatClassEntity?.seatClass
        bookingConfirmationVC.seatClassType = Myticket[indexPath.row].seat?.seatClass
        bookingConfirmationVC.headerInformation = Myticket[indexPath.row].head
        bookingConfirmationVC.headerInformation?.seatNumber = Myticket[indexPath.row].seat?.name
     
        bookingConfirmationVC.headerInformation?.price = headerInformation?.price

        bookingConfirmationVC.thistrainSchedule = trainSchedule
        bookingConfirmationVC.seatClassEntity = Myticket[indexPath.row].seat
       
        bookingConfirmationVC.Index = indexPath.row
        bookingConfirmationVC.mainvc = self
