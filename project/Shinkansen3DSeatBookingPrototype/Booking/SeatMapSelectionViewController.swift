...
class SeatMapSelectionViewController: BookingViewController {
    ...
    private var selectedEntity: ReservableEntity? {
        didSet {
           ...
           headerInformation?.carNumber = selectedEntity?.carNumber
            ...
            let buttonTitle = "Continue with seat"
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        setupStaticContent()
        //seatClass?.price = (seatClass?.price ?? 50)/50
        //seatMapSceneView.isHidden = true
    }
    ...
    @objc func mainCallToActionButtonDidTouch(_ sender: Button) {
        ...
        bookingConfirmationVC.headerInformation?.price = "\((seatClass?.price ?? 50)/50)"

        bookingConfirmationVC.thistrainSchedule = thistrainSchedule
        bookingConfirmationVC.seatClassEntity = seatClassEntity
